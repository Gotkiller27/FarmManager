...
// 1. Filtrer uniquement les AGENTS + Recherche
...
// Pagination basée sur les agents filtrés
const currentPage = ref(1);
const itemsPerPage = 8;
const totalPages = computed(() => Math.ceil(filteredAgents.value.length / itemsPerPage) || 1);

const paginatedAgents = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return filteredAgents.value.slice(start, start + itemsPerPage);
});
...
const deleteAgent = async (id) => {
  if (confirm("Supprimer cet agent du système ?")) {
...
<template>
  <div class="min-h-screen p-4 md:p-8 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white to-emerald-50 text-slate-800">
    ...
    <div class="max-w-7xl mx-auto mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div>
        <h1 class="text-4xl font-black text-emerald-900 tracking-tight">Nos Agents</h1>
        <p class="text-slate-500 mt-2 text-lg">Équipe de terrain AgriManage.</p>
      </div>
          ...
          <tbody>
            ...
            <tr v-for="user in paginatedAgents" :key="user.id" class="group transition-all duration-300">
              ...
              <td class="px-6 py-4 text-center bg-slate-50 group-hover:bg-emerald-50/50 transition-colors">
                <span class="inline-block px-3 py-1 rounded-lg text-[10px] font-black uppercase bg-white text-blue-600 border border-blue-100 shadow-sm">
                  Agent Terrain
                </span>
              </td>
