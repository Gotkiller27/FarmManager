...
const fetchGerants = async () => {
  try {
    ...
    console.log('Fetched gerants:', data);
...
const fetchCurrentDeptGerant = async () => {
  try {
    const data = await departmentStore.fetchCurrentDeptGerant(2); // 2 = Pisciculture
    if (data) {
...
const fetchCampaigns = async () => {
  try {
    loading.value = true;
    await campaignStore.fetchDepartmentCampaigns(2);
  } catch (err) {
...
const submitAssignation = async () => {
  console.log('submitAssignation called');
  console.log('isAdmin:', isAdmin);
  console.log('selectedGerantId:', selectedGerantId.value);
  ...
  if (!selectedGerantId.value) {
    console.log('No gerant selected');
    return;
  }
...
const submitForm = async () => {
  ...
  // Debug logs
  console.log('isGerant.value:', isGerant.value);
  console.log('authStore.user:', authStore.user);
  console.log('currentGerant.value:', currentGerant.value);
  ...
  // Cas de sécurité : si l'utilisateur est gérant, on force le gérant courant sur lui-même
  if (isGerant.value) {
  ...
  form.departement_id = 2; // S'assurer que le département est correct
...
<template>
  <div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0">
      <div>
        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-[#065f46]">Département Pisciculture</h1>
        <p class="text-gray-500 text-xs sm:text-sm">Gérez vos cycles de production aquacole</p>
      </div>
    ...
    <div v-if="hasAccess">
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
        <div class="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl relative">
          ...
          <p class="text-gray-400 text-sm mb-8">Créez une nouvelle campagne de pisciculture.</p>
          ...
          <form @submit.prevent="submitForm" class="space-y-5">
            <div>
              ...
              <input v-model="form.nom" type="text" required
                ...
                placeholder="ex: Campagne Saumon 2024">
            </div>
      ...
      <div v-if="showAssignModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
        <div class="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl relative">
          ...
          <p class="text-gray-400 text-sm mb-8">Sélectionnez le gérant pour le département Pisciculture.</p>
          ...
          <form @submit.prevent="submitAssignation" class="space-y-5">
            <div>
              ...
              <select v-model="selectedGerantId" @change="console.log('selectedGerantId changed to:', selectedGerantId)" required
